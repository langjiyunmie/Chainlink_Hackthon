...
contract MEVGuard is IMEVGuard, Ownable {
    ...
    // transient originTo：暂存发起地址 to，比如交易对合约调用时可能将 tx.origin 或者 msg.sender 记录到此
    address public transient originTo;

    // 实例化 SubcriptionConsumer 合约，用于获取随机数
    SubcriptionConsumer public subscriptionConsumer;
        ...
        returns (bool)
    {
        ...
        if (currentBlockNum < antiFrontDefendBlockEdge) {
            ...
            if (randomNum % denominator == 0) {
                // 注意：这里是比较操作，并没有真正赋值，合约里应为 =true，而不是 ==true
